...
const ExchangePop2 = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://weloveesc.xukaiteo.com:8000/credit/add', {
        method: 'POST',
        headers: {
        ...
        <div className='w-full flex flex-col items-center'>
          <form onSubmit={handleSubmit} className="w-full flex flex-col items-center">
            ...
            <h3 className="w-2/3 text-left bg-white mb-4 pl-1">
              Star points available: {starpoints ? starpoints : 'N/A'}
            </h3>
            <div className='w-1/2 text-left bg-white pl-1 mt-3'>
              <p>Star points used:</p>
              <input className='w-full text-center bg-4 pl-1 mb-4 rounded-lg'
                type="number" value={pointsUsed} onChange={handlePointsChange} min="0" max={starpoints} />
            </div>
